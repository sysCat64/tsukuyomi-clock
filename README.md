...
- GitHub Pages: `https://syscat64.github.io/tsukuyomi-clock/`
- 通常表示: `/`
- 稽古パネル: `/?keiko=1`
...
`main` へのpushで `.github/workflows/deploy-pages.yml` が走り、`dist/` をGitHub Pagesへ公開します。

GitHubリポジトリ側では、Settings -> Pages -> Source を `GitHub Actions` にしてください。

GitHub Pagesではリポジトリ名配下で配信されるため、Actions上では `GITHUB_PAGES=true` を設定してViteの `base` を `/tsukuyomi-clock/` に切り替えます。ローカルでPages向けの出力を確認する場合は以下を使います。
