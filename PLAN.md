...
- 現状はPhase 2で機構重視の画面polish、Phase 3で `?keiko=1` の隠し稽古パネル、Phase 4で静的成果物としてのREADME/QA/メタ情報整備を行った。Phase 5では、意味情報をHTML/SVGに残したまま、薄いラスタ素材で墨雲・山水・紙ムラ・月面・歯車影・円相筆致・和紙小窓・朱印・黒漆ケース擦れを補助する。
...
- Phase 5では、`src/assets/tsukuyomi-washi-landscape.jpg` を軽量な薄い背面レイヤーとして合成し、背景の墨雲、山水、枝、和紙のムラをラスタ素材で補助する。さらに `src/assets/tsukuyomi-enso-brush-overlay.png`、`src/assets/tsukuyomi-moon-surface-overlay.png`、`src/assets/tsukuyomi-gear-shadow-overlay.png` をSVG時計の装飾画像として合成し、円相の乾いた筆致、月窓の藍の面、機構影の密度を上げる。加えて `src/assets/tsukuyomi-panel-washi-overlay.png`、`src/assets/tsukuyomi-vermilion-seal-overlay.png`、`src/assets/tsukuyomi-flip-scuff-overlay.png` をHTMLパネル/パタパタケースの装飾レイヤーとして重ね、和紙の小窓、落款の押し跡、黒漆の擦れを補助する。時計針、暦、都市操作、天体値などの意味情報と操作面は引き続きHTML/SVG/React stateで保持する。
