...
- 現状はPhase 2で機構重視の画面polish、Phase 3で `?keiko=1` の隠し稽古パネル、Phase 4で静的成果物としてのREADME/QA/メタ情報整備を行った。Phase 5では、意味情報をHTML/SVGに残したまま、薄いラスタ背景で墨雲・山水・紙ムラを補助する。
...
- Phase 5では、`src/assets/tsukuyomi-washi-landscape.png` を薄い背面レイヤーとして合成し、背景の墨雲、山水、枝、和紙のムラをラスタ素材で補助する。時計針、暦、都市操作、天体値などの意味情報と操作面は引き続きHTML/SVG/React stateで保持する。
